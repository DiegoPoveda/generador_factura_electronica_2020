class FirmadorV2{
  ...
  public function firmar($certificadop12, $clavecertificado, $xmlsinfirma, $UUID){
      ...
      return $this->insertaFirma($xmlsinfirma);
  }
  ...
  public function insertaFirma($xml){
      ...
      $certSerialNumber = "7455089088839785380";
      $certDigest = "c2h3L+sr30SKmc7jo2+ia1wVZTm2Zd+59JTnQuOItRQ=";      
      $certIssuer = "C=CO, L=Bogota D.C., O=Andes SCD., OU=Division de certificacion entidad final, CN=CA ANDES SCD S.A. Clase II, emailAddress=info@andesscd.com.co";

      $SignedProperties = $this->generateSignedProperties($signTime,$certDigest,$certIssuer,$certSerialNumber);
      $SignedPropertiesWithSchemas = str_replace('<xades:SignedProperties', '<xades:SignedProperties '.$this->get_schemas(), $SignedProperties);
      $SignedPropertiesDigest = $this->base64Encode($SignedPropertiesWithSchemas);

      $KeyInfo = $this->getKeyInfo();
      $keyInfoWithShemas = str_replace('<ds:KeyInfo', '<ds:KeyInfo '.$this->get_schemas(), $KeyInfo);
      ...
      $SignedInfoWithSchemas = str_replace('<ds:SignedInfo', '<ds:SignedInfo '.$this->get_schemas(), $signedInfo);
